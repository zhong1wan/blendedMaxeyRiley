...
eta_video = VideoWriter('../trajectory_data/particle1000_posdensity.mp4','MPEG-4');
...
for tt=timestart:1:times
    ...
    if (tt == timestart)
        subplot(1,2,1);
        nt = hist3(squeeze(xt(idx_part,:,:)), 'Nbins', [128, 128]);
        hf1 = pcolor(x, y, nt/p);
        set(hf1,'EdgeColor','none');
        title('truth','interpreter','latex','fontsize',16);
        
        subplot(1,2,2);
        np = hist3(squeeze(xp(idx_part,:,:)), 'Nbins', [128, 128]);
        hf2 = pcolor(x, y, np/p);
